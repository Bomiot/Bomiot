...
class DriverList(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return models.Driver.objects.filter(query_conditions).order_by(ordering)
...
class DriverCreate(ModelViewSet):
    """
    ...
    ordering_fields = ['id', "created_time", "updated_time", ]
    filter_class = filter.DriverFilter
    queryset = models.Driver.objects.filter(is_delete=False)
            ...
            with transaction.atomic():
                ...
                for receiver, response in responses:
                    ...
                    if isinstance(response, dict) and response.get("msg"):
                        ...
                        models.Driver.objects.create(data=data, project=project_name)
...
class DriverUpdate(ModelViewSet):
    """
    A powerful ViewSet for database management and feedback mechanism
    """
    filter_backends = [DjangoFilterBackend, OrderingFilter, ]
    ...
    def update(self, request, *args, **kwargs):
        ...
        if project_name.lower() == 'bomiot':
            project_name = settings.PROJECT_NAME
        db_data = models.Driver.objects.filter(id=data.get('id'), is_delete=False)
...
class DriverDelete(ModelViewSet):
    """
    A powerful ViewSet for database management and feedback mechanism
    """
    filter_backends = [DjangoFilterBackend, OrderingFilter, ]
