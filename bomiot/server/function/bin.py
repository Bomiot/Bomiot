...
class BinList(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return models.Bin.objects.filter(query_conditions).order_by(ordering)

    def get_serializer_class(self):
        if self.action in ['list']:
            return serializers.BinSerializer
        else:
...
class BinCreate(ModelViewSet):
    ...
    ordering_fields = ['id', "created_time", "updated_time", ]
    filter_class = filter.BinFilter
    queryset = models.Bin.objects.filter(is_delete=False)
            ...
            with transaction.atomic():
                ...
                for receiver, response in responses:
                    ...
                    if isinstance(response, dict) and response.get("msg"):
                        ...
                        models.Bin.objects.create(data=data, project=project_name)
...
class BinUpdate(ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if project_name.lower() == 'bomiot':
            project_name = settings.PROJECT_NAME
        db_data = models.Bin.objects.filter(id=data.get('id'), is_delete=False)
