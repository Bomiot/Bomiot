...
class ASNDetailList(ModelViewSet):
    """
        list:
            Response a ASNDetail data list（all）
    """
    pagination_class = DataCorePageNumberPagination
    filter_backends = [DjangoFilterBackend, OrderingFilter, ]
    ordering_fields = ["id", "created_time", "updated_time", ]
    filter_class = filter.ASNDetailFilter
    ...
    def get_queryset(self):
        ...
        return models.ASNDetail.objects.filter(query_conditions).order_by(ordering)

    def get_serializer_class(self):
        if self.action in ['list']:
            return serializers.ASNDetailSerializer
        else:
...
class ASNDetailCreate(ModelViewSet):
    """
    ...
    ordering_fields = ['id', "created_time", "updated_time", ]
    filter_class = filter.ASNDetailFilter
    queryset = models.ASNDetail.objects.filter(is_delete=False)
            ...
            with transaction.atomic():
                ...
                for receiver, response in responses:
                    ...
                    if isinstance(response, dict) and response.get("msg"):
                        ...
                        models.ASNDetail.objects.create(data=data, project=project_name)
...
class ASNDetailUpdate(ModelViewSet):
    """
    A powerful ViewSet for database management and feedback mechanism
    """
    filter_backends = [DjangoFilterBackend, OrderingFilter, ]
    ...
    def update(self, request, *args, **kwargs):
        ...
        if project_name.lower() == 'bomiot':
            project_name = settings.PROJECT_NAME
        db_data = models.ASNDetail.objects.filter(id=data.get('id'), is_delete=False)
...
class ASNDetailDelete(ModelViewSet):
    """
    A powerful ViewSet for database management and feedback mechanism
    """
    filter_backends = [DjangoFilterBackend, OrderingFilter, ]
