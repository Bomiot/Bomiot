...
class FeeList(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return models.Fee.objects.filter(query_conditions).order_by(ordering)
...
class FeeCreate(ModelViewSet):
    ...
    ordering_fields = ['id', "created_time", "updated_time", ]
    filter_class = filter.FeeFilter
    queryset = models.Fee.objects.filter(is_delete=False)
            ...
            with transaction.atomic():
                ...
                for receiver, response in responses:
                    ...
                    if isinstance(response, dict) and response.get("msg"):
                        ...
                        models.Fee.objects.create(data=data, project=project_name)
...
class FeeUpdate(ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if project_name.lower() == 'bomiot':
            project_name = settings.PROJECT_NAME
        db_data = models.Fee.objects.filter(id=data.get('id'), is_delete=False)
