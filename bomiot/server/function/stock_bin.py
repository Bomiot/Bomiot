...
class StockBinList(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return models.StockBin.objects.filter(query_conditions).order_by(ordering)

    def get_serializer_class(self):
        if self.action in ['list']:
            return serializers.StockBinSerializer
        else:
...
class StockBinCreate(ModelViewSet):
    """
    ...
    ordering_fields = ['id', "created_time", "updated_time", ]
    filter_class = filter.StockBinFilter
    queryset = models.StockBin.objects.filter(is_delete=False)
            ...
            with transaction.atomic():
                ...
                for receiver, response in responses:
                    ...
                    if isinstance(response, dict) and response.get("msg"):
                        ...
                        models.StockBin.objects.create(data=data, project=project_name)
...
class StockBinUpdate(ModelViewSet):
    """
    A powerful ViewSet for database management and feedback mechanism
    """
    filter_backends = [DjangoFilterBackend, OrderingFilter, ]
    ...
    def update(self, request, *args, **kwargs):
        ...
        if project_name.lower() == 'bomiot':
            project_name = settings.PROJECT_NAME
        db_data = models.StockBin.objects.filter(id=data.get('id'), is_delete=False)
...
class StockBinDelete(ModelViewSet):
    """
    A powerful ViewSet for database management and feedback mechanism
    """
    filter_backends = [DjangoFilterBackend, OrderingFilter, ]
