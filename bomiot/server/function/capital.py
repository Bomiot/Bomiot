...
class CapitalList(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return models.Capital.objects.filter(query_conditions).order_by(ordering)

    def get_serializer_class(self):
        if self.action in ['list']:
            return serializers.CapitalSerializer
        else:
...
class CapitalCreate(ModelViewSet):
    """
    ...
    ordering_fields = ['id', "created_time", "updated_time", ]
    filter_class = filter.CapitalFilter
    queryset = models.Capital.objects.filter(is_delete=False)
            ...
            with transaction.atomic():
                ...
                for receiver, response in responses:
                    ...
                    if isinstance(response, dict) and response.get("msg"):
                        ...
                        models.Capital.objects.create(data=data, project=project_name)
...
class CapitalUpdate(ModelViewSet):
    """
    A powerful ViewSet for database management and feedback mechanism
    """
    filter_backends = [DjangoFilterBackend, OrderingFilter, ]
    ...
    def update(self, request, *args, **kwargs):
        ...
        if project_name.lower() == 'bomiot':
            project_name = settings.PROJECT_NAME
        db_data = models.Capital.objects.filter(id=data.get('id'), is_delete=False)
...
class CapitalDelete(ModelViewSet):
    """
    A powerful ViewSet for database management and feedback mechanism
    """
    filter_backends = [DjangoFilterBackend, OrderingFilter, ]
