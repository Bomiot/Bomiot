...
class DNDetailList(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return models.DNDetail.objects.filter(query_conditions).order_by(ordering)

    def get_serializer_class(self):
        if self.action in ['list']:
            return serializers.DNDetailSerializer
        else:
...
class DNDetailCreate(ModelViewSet):
    """
    ...
    ordering_fields = ['id', "created_time", "updated_time", ]
    filter_class = filter.DNDetailFilter
    queryset = models.DNDetail.objects.filter(is_delete=False)
            ...
            with transaction.atomic():
                ...
                for receiver, response in responses:
                    ...
                    if isinstance(response, dict) and response.get("msg"):
                        ...
                        models.DNDetail.objects.create(data=data, project=project_name)
...
class DNDetailUpdate(ModelViewSet):
    """
    A powerful ViewSet for database management and feedback mechanism
    """
    filter_backends = [DjangoFilterBackend, OrderingFilter, ]
    ...
    def update(self, request, *args, **kwargs):
        ...
        if project_name.lower() == 'bomiot':
            project_name = settings.PROJECT_NAME
        db_data = models.DNDetail.objects.filter(id=data.get('id'), is_delete=False)
...
class DNDetailDelete(ModelViewSet):
    """
    A powerful ViewSet for database management and feedback mechanism
    """
    filter_backends = [DjangoFilterBackend, OrderingFilter, ]
