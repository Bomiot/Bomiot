...
class DNList(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return models.DN.objects.filter(query_conditions).order_by(ordering)

    def get_serializer_class(self):
        if self.action in ['list']:
            return serializers.DNSerializer
        else:
...
class DNCreate(ModelViewSet):
    ...
    ordering_fields = ['id', "created_time", "updated_time", ]
    filter_class = filter.DNFilter
    queryset = models.DN.objects.filter(is_delete=False)
            ...
            with transaction.atomic():
                ...
                for receiver, response in responses:
                    ...
                    if isinstance(response, dict) and response.get("msg"):
                        ...
                        models.DN.objects.create(data=data, project=project_name)
...
class DNUpdate(ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if project_name.lower() == 'bomiot':
            project_name = settings.PROJECT_NAME
        db_data = models.DN.objects.filter(id=data.get('id'), is_delete=False)
